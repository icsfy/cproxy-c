...
int setup_iptables(pid_t pid) {
    ...
    if (g_ctx.mode == MODE_REDIRECT) {
        ...
        CHECK(run_cmd("iptables -w -t nat -A %s -p tcp %s -j REDIRECT --to-ports %d", g_ctx.output_chain, cg_match, g_ctx.port));
        if (g_ctx.redirect_dns) {
            CHECK(run_cmd("iptables -w -t nat -A %s -p udp %s --dport 53 -j REDIRECT --to-ports %d", g_ctx.output_chain, cg_match, g_ctx.port));
            CHECK(run_cmd("iptables -w -t nat -A %s -p tcp %s --dport 53 -j REDIRECT --to-ports %d", g_ctx.output_chain, cg_match, g_ctx.port));
        }
        run_cmd_silent("ip6tables -w -t raw -A %s %s -j DROP", out6_chain, cg_match);
        ...
        CHECK(init_chain("mangle", pre6_chain, "PREROUTING", "ip6tables"));
        CHECK(apply_bypass_rules(g_ctx.bypass_str, pre6_chain, "mangle", "ip6tables"));
        CHECK(run_cmd("ip6tables -w -t mangle -A %s -p udp -m mark --mark %d -j TPROXY --on-ip ::1 --on-port %d", pre6_chain, g_ctx.tproxy_mark, g_ctx.port));
        CHECK(run_cmd("ip6tables -w -t mangle -A %s -p tcp -m mark --mark %d -j TPROXY --on-ip ::1 --on-port %d", pre6_chain, g_ctx.tproxy_mark, g_ctx.port));
        ...
        CHECK(init_chain("mangle", out6_chain, "OUTPUT", "ip6tables"));
        ...
        CHECK(run_cmd("iptables -w -t mangle -A %s -p tcp %s -j MARK --set-mark %d", g_ctx.output_chain, cg_match, g_ctx.tproxy_mark));
        CHECK(run_cmd("iptables -w -t mangle -A %s -p udp %s -j MARK --set-mark %d", g_ctx.output_chain, cg_match, g_ctx.tproxy_mark));
        CHECK(run_cmd("ip6tables -w -t mangle -A %s -p tcp %s -j MARK --set-mark %d", out6_chain, cg_match, g_ctx.tproxy_mark));
        CHECK(run_cmd("ip6tables -w -t mangle -A %s -p udp %s -j MARK --set-mark %d", out6_chain, cg_match, g_ctx.tproxy_mark));
        if (g_ctx.has_override_dns) {
            CHECK(run_cmd("iptables -w -t nat -A %s -p udp %s --dport 53 -j DNAT --to-destination %s", dns_chain, cg_match, g_ctx.override_dns));
            CHECK(run_cmd("iptables -w -t nat -A %s -p tcp %s --dport 53 -j DNAT --to-destination %s", dns_chain, cg_match, g_ctx.override_dns));
        }
        ...
        CHECK(run_cmd("iptables -w -t raw -A %s %s -j LOG --log-prefix \"cproxy: \"", g_ctx.output_chain, cg_match));
        run_cmd_silent("ip6tables -w -t raw -A %s %s -j LOG --log-prefix \"cproxy: \"", out6_chain, cg_match);
    }
    return 0;
}
...
void cleanup_iptables(void) {
    ...
    if (g_ctx.mode == MODE_REDIRECT) {
        ...
        run_cmd_silent("ip6tables -w -t mangle -D PREROUTING -j %s", pre6_chain);
        run_cmd_silent("ip6tables -w -t mangle -F %s", pre6_chain);
        run_cmd_silent("ip6tables -w -t mangle -X %s", pre6_chain);

        run_cmd_silent("ip6tables -w -t mangle -D OUTPUT -j %s", out6_chain);
